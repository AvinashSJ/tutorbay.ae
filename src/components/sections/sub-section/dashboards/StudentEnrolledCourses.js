...
const StudentEnrolledCourses = () => {
  ...
  const enrolledCourses = userPosts?.filter(({ status }) => status === "open");
  const activeCourses = userPosts?.filter(({ status }) => status === "active");
  const completedCourses = userPosts?.filter(
    ({ status }) => status === "close"
  );
